...
dbSendQuery(con, "CREATE TABLE tree AS SELECT
  CN AS TREE_CN,
  PLT_CN,
  PREV_TRE_CN,
  INVYR,
...
FROM 'data/rawdat/state/*_TREE.csv'
WHERE (INVYR >= 2000.0)")

dbSendQuery(con, "CREATE TABLE plot AS SELECT
  CN AS PLOT_CN,
  SRV_CN,
  CTY_CN,
  PREV_PLT_CN,
  ...
  REUSECD3,
  CONCAT_WS('_', STATECD, UNITCD, COUNTYCD, PLOT) AS PLOT_COMPOSITE_ID
FROM 'data/rawdat/state/*_PLOT.csv'
WHERE (INVYR >= 2000.0)")

dbSendQuery(con, "CREATE TABLE cond AS SELECT
   CN AS COND_CN,
  PLT_CN,
  INVYR,
  STATECD,
  ...
  CONCAT_WS('_', STATECD, UNITCD, COUNTYCD, PLOT) AS PLOT_COMPOSITE_ID
FROM 'data/rawdat/state/*_COND.csv'
WHERE (INVYR >= 2000.0)")
